"""Warp-level dual quant: 32x32 tile, row+col in one kernel.

Algorithm:
- Load (32, 32) tile once
- Row amax: tl.max(abs, axis=1) → (32,)
- Col amax: tl.max(abs, axis=0) → (32,)
- Row quant: values * row_scale[:, None] → row output (coalesced)
- Col quant: values * col_scale[None, :] → transpose → col output
- Single HBM read, two fp8 writes, minimal registers
"""
...
def _e8m0(amax):
    ...
    e = exp - 8 + carry
    e = tl.where(exp > 0, e, 0)
    e_byte = tl.maximum(e, 0).to(tl.uint8)
    qe = tl.maximum(tl.minimum(254 - e, 254), 1)
    qs = (qe.to(tl.int32) << 23).to(tl.float32, bitcast=True)
    return e_byte, qs


@triton.jit
def _warp_dual_quant_kernel(
    ...
    for gb in tl.range(0, GROUPS_PER_BLOCK):
        ...
        # ─── Row quant (axis=1: groups along dim) ───
        ra = tl.max(av, axis=1)
        re, rs = _e8m0(ra)
        rq = (v * rs[:, None]).to(tl.float8e4nv)
        tl.store(row_fp8_ptr + sr[:, None] * H + dim_offs[None, :], rq, mask=dim_mask[None, :])

        # Row ISA: vectorized over 32 rows
        srt = sr // SF_TILE_M
        sri = sr % SF_TILE_M
        ...
        tl.store(row_scales_ptr + rtb + srb + rkn, re)

        # ─── Col quant (axis=0: groups along tokens) ───
        ca = tl.max(av, axis=0)
        ce, cs = _e8m0(ca)
        cq = (v * cs[None, :]).to(tl.float8e4nv)
        ct = tl.trans(cq)  # (32 dims, 32 tokens)
...
def warp_dual_quant(src, num_experts, capacity, *, gather_idx=None):
    src = src.contiguous()
    T, H = src.shape
    device = src.device
    GS = _SF_VEC_SIZE
    ...
    _warp_dual_quant_kernel[grid](
        ...
        GROUPS_PER_BLOCK=GPB, num_warps=1,
    )
...
if __name__ == "__main__":
    ...
    try:
        r1, s1, c1, s2 = warp_dual_quant(dout, E, CAP, gather_idx=x_idx)
        torch.cuda.synchronize()
        print("Compiled OK")
    ...
    print(f"Row fp8: {rm*100:.2f}%  Col fp8: {cm*100:.2f}%")
    ...
    t_dual = bench(lambda: warp_dual_quant(dout, E, CAP, gather_idx=x_idx),
                   "Warp dual quant (1 kernel, row+col)")
    print(f"\n  {t_sep:.0f}us → {t_dual:.0f}us  ({t_sep/t_dual:.2f}x)")
