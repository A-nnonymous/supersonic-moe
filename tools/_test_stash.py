"""Test stash_bf16_to_cpu: correctness + memory."""
...
# Sync grads from proxy to Parameter
moe.unstash_bf16()
moe.sync_grads()
...
print("=== CORRECTNESS (stash vs no-stash) ===")
for name, ref, test in [("output", ref_out, test_out), ("dx", ref_dx, test_dx),
                         ("dw1", ref_dw1, test_dw1), ("dw2", ref_dw2, test_dw2)]:
    if test is None:
        print(f"  {name:10s}  NONE")
    else:
        diff = (ref - test).abs().max().item()
        print(f"  {name:10s}  {'BIT-IDENTICAL' if diff == 0 else f'max_diff={diff:.2e}'}")
