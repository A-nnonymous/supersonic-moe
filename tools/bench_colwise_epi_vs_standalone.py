#!/usr/bin/env python
"""Microbench: colwise epi quant vs standalone colwise_quantize_and_pack.

At production shape T=8192 E=8 K=4 H=2048 I=1408 (TK_padded, N2=2*I=2816).

Compares:
- Path A: BF16-D GEMM + standalone colwise_quantize_and_pack (current prod)
- Path B: FP8-D GEMM with epi-fused colwise quant
- Path C: BF16-D GEMM alone (reference floor)
"""
...
def main():
    ...
    from sonicmoe.quack_utils.blockscaled_fp8_gemm import (
        ...
        _gather_isa_packed_scales_kernel, colwise_quantize_and_pack,
    ...
    def path_a():
        ...
        return colwise_quantize_and_pack(z, logical_rows=N2, logical_cols=TK)

    def path_b():
        return blockscaled_fp8_gemm_zeromat_colwise_quant(
            x_fp8, w1_fp8.mT, cu_seqlens_m=eFO, A_idx=x_gather_idx,
            a_scales=x_scales_tk_e8m0, b_scales=w1_scales)
    ...
    t_c = bench(path_c)
    t_quant = t_a - t_c
    t_epi_overhead = t_b - t_c
    savings = t_a - t_b
    print(f"  Path A (BF16 GEMM + standalone colwise quant): {t_a:7.1f} µs")
    print(f"  Path B (FP8 GEMM + epi-fused colwise quant):   {t_b:7.1f} µs")
    print(f"  Path C (BF16 GEMM alone):                       {t_c:7.1f} µs")
    print(f"  Standalone colwise quant cost (A−C):            {t_quant:7.1f} µs")
    print(f"  Epi overhead on GEMM   (B−C):                   {t_epi_overhead:7.1f} µs")
    print(f"  REALIZABLE SAVINGS (A−B):                       {savings:7.1f} µs ({100*savings/t_a:.1f}%)")
